...
def _wrap_handle_message(orig: Callable) -> Callable:
    ...
    async def wrapper(self, event, *args, **kwargs):
        # v1.6.1-fix: platform guard — only Feishu messages may enter the
        # Feishu card pipeline. Telegram/Discord/etc. use numeric message ids
        # that Feishu's CardKit API rejects ("Invalid ids"), which caused card
        # creation to fail and the reply to degrade into chopped-up plain text.
        try:
            _src = getattr(event, "source", None)
            _p = getattr(_src, "platform", None)
            _plat = getattr(_p, "value", _p) or ""
            _plat = str(_plat).lower()
            if _plat and _plat != "feishu":
                return await orig(self, event, *args, **kwargs)
        except Exception:
            _logger.debug("HLS: platform guard probe failed", exc_info=True)
...
def _wrap_handle_message_with_agent(orig: Callable) -> Callable:
    ...
    async def wrapper(self, event, source, *args, **kwargs):
        # v1.6.1-fix: platform guard — only Feishu messages may enter the
        # Feishu card pipeline. Telegram/Discord/etc. use numeric message ids
        # that Feishu's CardKit API rejects ("Invalid ids"); without this guard
        # their messages created Feishu cards, the API call failed, and the
        # reply degraded to chopped-up plain text ("一段一段").
        try:
            _p = getattr(source, "platform", None)
            _plat = getattr(_p, "value", _p) or ""
            _plat = str(_plat).lower()
            if _plat and _plat != "feishu":
