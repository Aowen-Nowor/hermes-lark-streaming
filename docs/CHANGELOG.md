...
| 🐛 Bug Fix (P1) | **AGENT_GUIDE "方式三 pip install" 指向第三方衍生包**：PyPI 存在同名牌 `hermes-lark-streaming` 2.4.0——实为第三方账号（monkey2jack）上传的衍生包（fork 本仓库 10 个核心文件字节级相同 + 自有"泡波"主题层，产品名 aiduPOP），包元数据署名却是 Aowen-Nowor。照文档执行会装到它而非本插件 | 用户已确认非本人发布；本插件官方渠道只有 Gitee/GitHub 仓库 | ①AGENT_GUIDE 方式三整体改写为警示区（说明冒名包身份 + 官方渠道仅方式一/二 + 误装卸载指引）；②README 双语在 Cheerwhy 不兼容警示旁新增 🚫 PyPI 冒名包警示框 (`docs/AGENT_GUIDE.md`, `README.md`, `README.zh-CN.md`) |
