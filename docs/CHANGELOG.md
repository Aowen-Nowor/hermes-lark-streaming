...
| ✨ Feature | 新增 `width_mode` 卡片宽度配置 | CardKit 支持 `width_mode` 配置：`default`(600px)、`compact`(400px)、`fill`(撑满窗口)。通过 CardKit ApiConfig 设置 | 新增 `width_mode` 配置项（默认 `default`），传入 `build_streaming_card_v2`/`build_static_card_v2` → CardKit `config.width_mode`。provider 卡片 footer 也受支持 (`config/reader.py`, `cardkit/cards.py`) |
